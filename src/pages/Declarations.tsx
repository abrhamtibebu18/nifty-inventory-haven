...
export default function Declarations() {
  ...
  const [incompleteDeclarations, setIncompleteDeclarations] = useState<DeclarationFile[]>([
    {
      id: "1",
      status: "incomplete",
      date: "2023-04-15",
  ...
  const [completeDeclarations, setCompleteDeclarations] = useState<DeclarationFile[]>([
    {
      id: "7",
      status: "complete",
      date: "2023-01-31",
  ...
  const filteredIncompleteDeclarations = incompleteDeclarations.filter(dec => 
    dec.title.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const filteredCompleteDeclarations = completeDeclarations.filter(dec => 
    dec.title.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      <div className="container py-6">
        <Tabs defaultValue="incomplete" className="w-full">
          ...
          <TabsContent value="incomplete" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredIncompleteDeclarations.length > 0 ? (
              filteredIncompleteDeclarations.map((file) => (
                  ...
                  <div className="pt-10">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button variant="outline" size="sm" className="flex-1" asChild>
                        <Link to={`/declarations/incomplete`}>
              ...
              <div className="col-span-3 py-10 text-center text-muted-foreground">
                No incomplete declarations found matching your search criteria
              </div>
            )}
          </TabsContent>
          
          <TabsContent value="complete" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCompleteDeclarations.length > 0 ? (
              filteredCompleteDeclarations.map((file) => (
              ...
              <div className="col-span-3 py-10 text-center text-muted-foreground">
                No complete declarations found matching your search criteria
              </div>
            )}
          </TabsContent>
        </Tabs>
